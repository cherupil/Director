(() => {
  ...
  var _addToTimeline = function _addToTimeline2(timeline3, child, position, skipChecks) {
    ...
    child._start = _round((_isNumber(position) ? position : position || timeline3 !== _globalTimeline ? _parsePosition(timeline3, position, child) : timeline3._time) + child._delay);
  ...
  var ringColor = colors[Math.floor(Math.random() * colors.length)];
  var ring = {
    angle: -Math.PI / 2,
  ...
  var crossBarColor = colors[Math.floor(Math.random() * colors.length)];
  var crossBarX = {
    start: 0,
    end: 0
  };
  ...
  var timeline2 = new gsapWithCSS.timeline();
  composition.from(circles, { scale: 0 }, { duration: 2.4, ease: "easeOutExpo", stagger: 0.1, onComplete: () => {
    console.log("done");
  } });
  ...
  var contentHeight = content.offsetHeight - window.outerHeight;
